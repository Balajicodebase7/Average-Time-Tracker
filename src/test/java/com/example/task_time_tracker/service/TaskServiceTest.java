...
class TaskServiceTest {
    ...
    void recordTaskPerformed_newTask() {
        ...
        // We can't easily verify the state of 'newTaskDetails' without a captor or custom matcher
        ...
        verify(taskDetailsRepository).save(argThat(task ->
            task.getTaskId().equals(taskId) &&
            task.getDurationTaken() == duration &&
            task.getTaskCount() == 1
        ));
    }
    ...
    void recordTaskPerformed_existingTask() {
        ...
        // For this test, we assume TaskDetails is constructed and then addDuration is called by the service.
        // So, let's set it up so addDuration works from a base.
        // Corrected approach: set the state on the mock *before* addDuration is called by the service
        ...
        verify(taskDetailsRepository).save(argThat(task ->
            task.getTaskId().equals(taskId) &&
            task.getDurationTaken() == (initialDuration + newDuration) &&
